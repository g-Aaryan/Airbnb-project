...
func SetupRouter(UserRouter Router, RoleRouter Router) *chi.Mux {
	...
	chiRouter.Group(func(r chi.Router) {
		...
		bookingServiceURL := env.GetString("BOOKING_SERVICE_URL", "http://localhost:3007")
		hotelServiceURL := env.GetString("HOTEL_SERVICE_URL", "http://localhost:3006")
		notificationServiceURL := env.GetString("NOTIFICATION_SERVICE_URL", "http://localhost:3005")
		reviewServiceURL := env.GetString("REVIEW_SERVICE_URL", "http://localhost:3004")
